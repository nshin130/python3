...
# 자동차 클래스 정의
# 차라고 인지할 만한 특성 정의:
# size, color, wheels, doors, isLoad

class Car:
    def __init__(self, size, color, wheels, doors, isLoad):  # constructor
        self.size = size
        self.color = color
        self.wheels = wheels
        self.doors = doors
    ...
    def __str__(self):  # 자바의 toString역할 생성자들을 몽땅 다 출력할수있게함
        return '우우우웅~'
...
class Employee:
    ...
    def __str__(self):
        result = f'{self.empno}, {self.lname}, {self.fname},' \
                 f' {self.email}, {self.phone}, {self.hdate}, ' \
                 f'{self.jonid}, {self.sal}, {self.comm}, ' \
                 f'{self.mgr}, {self.deptid}'
        return result


emp = Employee(123, 'King', 'Steven', 'steve@king.com', '123-456-789',
               '2003-06-170', 'Ad_PRES', 24000, '', '', 90)
print(emp)
